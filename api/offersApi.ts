...
export class OffersApi {
    ...
    public async offersList (xAccountToken: string, applicationId?: string, createdAfter?: Date, createdBefore?: Date, creatorId?: string, cursor?: string, expand?: 'application' | 'application,creator' | 'creator', includeRemoteData?: boolean, modifiedAfter?: Date, modifiedBefore?: Date, pageSize?: number, remoteId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PaginatedOfferList;  }> {
        ...
        if (creatorId !== undefined) {
            localVarQueryParameters['creator_id'] = ObjectSerializer.serialize(creatorId, "string");
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PaginatedOfferList;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "PaginatedOfferList");
    ...
    public async offersRetrieve (xAccountToken: string, id: string, expand?: 'application' | 'application,creator' | 'creator', includeRemoteData?: boolean, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Offer;  }> {
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: Offer;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "Offer");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
